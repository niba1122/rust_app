...
Module.STDWEB_PRIVATE.acquire_tmp = function( dummy ) {
    ...
    const __imports = {
        env: {
            ...
            "__extjs_0fb2a2d3b5e14ecaffa99211ad5f448ee31bf485": function($0, $1, $2, $3, $4, $5, $6) {
                $1 = Module.STDWEB_PRIVATE.to_js($1);$2 = Module.STDWEB_PRIVATE.to_js($2);$3 = Module.STDWEB_PRIVATE.to_js($3);$4 = Module.STDWEB_PRIVATE.to_js($4);$5 = Module.STDWEB_PRIVATE.to_js($5);$6 = Module.STDWEB_PRIVATE.to_js($6);Module.STDWEB_PRIVATE.from_js($0, (function(){var data = {method : ($1), body : ($2), headers : ($3),}; var request = new Request (($4), data); var callback = ($5); var handle = {active : true , callback ,}; fetch (request). then (function (response){var promise = (($6))? response.arrayBuffer (): response.text (); var status = response.status ; var headers = {}; response.headers.forEach (function (value , key){headers [key]= value ;}); promise.then (function (data){if (handle.active == true){handle.active = false ; callback (true , status , headers , data); callback.drop ();}}). catch (function (err){if (handle.active == true){handle.active = false ; callback (false , status , headers , data); callback.drop ();}});}); return handle ;})());
            },
    ...
    if( __load_asynchronously ) {
        return WebAssembly.instantiate( __wasm_module, __imports )
            .then( instance => {
                ...
                console.log( "Finished loading Rust wasm module 'rust_app'" );
                return Module.exports;
            })
            .catch( error => {
                console.log( "Error loading Rust wasm module 'rust_app':", error );
                throw error;
            });
    } else {
    ...
    if( typeof window === "undefined" && typeof process === "object" ) {
        ...
        const wasm_path = path.join( __dirname, "rust_app.wasm" );
        const buffer = fs.readFileSync( wasm_path );
        const mod = new WebAssembly.Module( buffer );

        return __initialize( mod, false );
    } else {
        return fetch( "rust_app.wasm" )
